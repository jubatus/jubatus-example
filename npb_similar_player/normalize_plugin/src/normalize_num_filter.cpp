...
const std::string& get_or_die(const std::map<std::string, std::string> & params,
  ...
  if (it == params.end()) {
    throw JUBATUS_EXCEPTION(jubatus::fv_converter::converter_exception(std::string("\"" + key +  "\" is not specified in parameters")));
  }
  return it->second;
}
...
extern "C" {
  jubatus::fv_converter::num_filter* create(std::map<std::string, std::string>& params) {
